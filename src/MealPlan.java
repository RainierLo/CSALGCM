...
/**
 * Team 4 CRUZADA, CARLA JEANNINE ORTEGA EVANGELISTA, INA PATRICIA DE LIMA
 * FRANCISCO, PIOLO JOSE TAN LOYOLA, RAINIER MAGNO
 * 
 * Stand-alone application that calculates the recommended amount of grams,
 * together with its associated calories, that a certain food portion should
 * only have to limit the number of calories of a certain meal. This will then
 * help in achieving the target calories set by the user for a planned meal.
 * 
 * The problem is reduced as a Knapsack Problem, and it was solved through
 * dynamic programming and using a greedy algorithm.
 * 
 * Greedy Algorithm is based from what has been taught in class.
 * 
 * Reference for the dynamic programming solution:
 * https://www.codesdope.com/course/algorithms-knapsack-problem/ The functions
 * mentioned in the comments for the said algorithm are also from this
 * reference.
 */
...
public class MealPlan {
    ...
    static private long startTimeGreedy;
    static private long stopTimeGreedy;
    static private long startTimeDynamic;
    static private long stopTimeDynamic;
    ...
    private void addItem(FoodItem item, JTextArea log) {
        String text = item.getFoodName() + " " + item.getGrams() + "g " + item.getCalories()
                + " calories added to the list!\n";
        items.add(item);
        log.append(text);
        clearField();
    }
    ...
    private void GreedyAlgo(JTextArea log) {
        ...
        startTimeGreedy = System.nanoTime();
        ...
        stopTimeGreedy = System.nanoTime();

        System.out.println(
                "Elapsed time for greedy algorithm was " + (stopTimeGreedy - startTimeGreedy) + " nanoseconds.");
    ...
    private void dynamicAlgo(JTextArea log) {
        ...
        int[] v = new int[n];
        int[] w = new int[n];
        int[][] cost = new int[n + 1][W + 1]; // would contain the optimal values for the first i items and weight limit
                                              // of j
        ...
        for (i = 1; i <= n; i++) {
            ...
            for (int k = 1; k <= W; k++) {
                if (w[i - 1] > k) { // if the current weight of the item is greater than k, we cannot add it to the
                                    // knapsack
                    cost[i][k] = cost[i - 1][k]; // only left with the i - 1 items and the weight limit W, leave the
                                                 // item
                } else { // we can add the item to the knapsack

                    /**
                     * Making the decision to add or leave the item depends on whichever has the
                     * maximum value between the function F(i-1, W) and F(i-1, W-wi) + vi. if the
                     * function F(i-1, W-wi) + vi is greater, the item will be picked, making the
                     * sum the optimal value of that item. Otherwise, the item will not be included.
                     */
        ...
        stopTimeDynamic = System.nanoTime();

        System.out.println(
                "Elapsed time for dynamic programming was " + (stopTimeDynamic - startTimeDynamic) + " nanoseconds.");

        dynamicItems(log, cost, n, w, v); // finalize the output of the matrix cost
    }

    /**
     * This method determines the items included in the result of the dynamic
     * programming. It does not include the duplicate optimal value between two rows
     * since the duplicate optimal value has no effect in the total optimized value.
     * 
     * @param log  is the area where the text will be displayed in the GUI.
